...
expectation-level mimicry floor and, when per-window estimates are independent
conditional on the strategy, an exponentially decreasing probability of sustained
realized evasion. It gives no single-window guarantee and does not cover
sub-resolution harm.
...
Tier-A dimensions requires about $404$\,bytes ($0.15\%$ of a 256\,KB static
...
per window and sampled-set means are independent across the two pools. Then
...
\textbf{A high-probability refinement, for exposure only.} For a fixed sequence
...
\[
  ...
  =0.579\%,
\]
where $q_0=0.5$, $\mu_C^{att}/\rmax=0.156$, and
...
illustrative 16-bit storage budget in \Cref{sec:overhead} is $404$\,bytes, or
$0.154\%$ of a 256\,KB SRAM. It is not an RTL area, energy, or timing result.
...
prefetcher-, replacement-, and scheduler-like controllers (\Cref{fig:p3}(a)).
...
auditor-observable features (\Cref{fig:p3}(b)). Five-fold cross-validation over
...
\caption{P3 synthetic abstractions. (a) The floor holds across three harness
instantiations; (b) attack and drift are separable within the constructed episode
...
\Cref{fig:theory}(a) checks the CUSUM constants on an i.i.d.\ Gaussian signal.
...
empirical/theory ratio is $0.998$, and at $H=5\sigma$ the false-alarm run length
...
\Cref{fig:theory}(b) separates the proved loose form from a descriptive tracker.
...
\caption{Theory. (a) ARL matches Siegmund; (b) measured regret versus the
...
means and standard deviations for $S_{in}$; 18 feature/action/bias coefficients
for $S_{res}$; four two-sided CUSUMs; and 64 set-dueling counters. The total is
$404$ bytes, or $0.154\%$ of a 256\,KB SRAM.
...
\midrule
\textbf{Total} & \textbf{404\,B} & estimator/gate off path\\
\bottomrule
\end{tabular}
\end{table}
...
$404$ bytes in an illustrative 16-bit storage count. RTL area, energy, timing,
