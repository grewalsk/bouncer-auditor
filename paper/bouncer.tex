...
buys is a \emph{sustained} guarantee: the assignment is redrawn independently each
epoch and rewards are bounded, so a truly degrading strategy---one with
$\mathbb{E}[\dhat_R]\le\tau-\epsilon$---keeps the $W$-window average of $\dhat_R$
above $\tau$ only with probability $\le e^{-2W\epsilon^2/r_{\max}^2}$ (Hoeffding);
and once that average falls below $\tau$, the CUSUM (reference $K=\tau+\gamma/2>\tau$)
accumulates at least $\gamma/2$ per window and fires within $2H/\gamma$ further
windows. Sustained realized evasion by a degrading adversary is thus exponentially
unlikely in the audit length. Under a leaked fraction $f$ of the within-$R$
...
\emph{(ii)} Per-window $\dhat_R$ values are independent across epochs (fresh secret
draws) and bounded, so Hoeffding bounds the probability that their $W$-window
average exceeds its mean by $\epsilon$; a strategy with
$\mathbb{E}[\dhat_R]\le\tau-\epsilon$ therefore keeps the average above $\tau$ w.p.\
$\le e^{-2W\epsilon^2/r_{\max}^2}$. When the average is below $\tau$, each window
adds at least $K-\tau=\gamma/2$ to the CUSUM statistic, which reaches $H$ within
$2H/\gamma$ windows. \emph{(Leak.)} Leaked knowledge of fraction $f$ lets the
...
windows of latency), detection power holds and then falls off a cliff as the secret leaks---
...
$\dhat_R$ stays $\ge\tau$ decays under the Hoeffding envelope $e^{-2W\epsilon^2/r_{\max}^2}$
